...
const Sidebar = () => {
  ...
  const sidebarClassNames = `fixed flex flex-col ${isSidebarCollapsed ? "w-4 md:w-16" : "w-72 md:w-64"} bg-white transition-all duration-300 overflow-hidden h-full shadow-md z-40`;
  return (
    <div className={sidebarClassNames}>
      {/*TOP LOGO*/}
      <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
          <Image src={Logo} alt='logo' className='size-6' />
          <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>PANELIO</h1>

        {/* <button className='md-hidden px-3 py-3 bg-gray-100 rounded-full gover:bg-blue-100' onClick={toggleSidebar} >
      ...
      <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
        <p  className='text-center text-xs text-gray-500'>&copy; 2024 Panel-io </p>
      </div>
    </div>
  )
}
